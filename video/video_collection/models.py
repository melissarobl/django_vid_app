...
class Video(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        url_components = parse.urlparse(self.url) # parse breaks URL into components
        ...
        query_string = url_components.query # 'v=123awe56' , takes out everything after the ? aka- the query
        if not query_string:
            raise ValidationError(f'Invalid YouTube URL {self.url}')
        parameters = parse.parse_qs(query_string, strict_parsing=True) # parse.parse_qs is from urllib.parse import and is designed to convert query string to dictionary
        v_parameters_list = parameters.get('v') # dictionary will have {'v': [123awe56]} and will return None if no key found, e.g. abc=1234
